    ...
    def count_persons_by_profession(self, threshold=2) -> DataFrame:
        ...
        return (
            self.tsv_df
                ...
                .groupBy(NameBasicsModel.primary_profession)
                .count()
                .limit(num=10)
        )
        ...
        return (
            self.tsv_df
                .filter(condition=(
                    ...
                    col(NameBasicsModel.primary_profession).isNotNull() &
                    (
                        col(NameBasicsModel.primary_profession).contains("actor")) |
                        (col(NameBasicsModel.primary_profession).contains("actress")
                    )
                ))
                ...
                .groupBy(actor_col, NameBasicsModel.known_for_titles)
                .agg(avg(TitleRatingsModel.average_rating).alias("avg_rating"))
